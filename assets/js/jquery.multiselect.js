(function($) {
	...
	$.fn.gs_multiselect = function(params) {
		...
		return this.each(function() {
			...
			var from_sel=$('<select multiple="multiple" style="color:black;"></select>').get(0);
			var r=t.insertRow(0);
			if (options.buttons) {
			...
			to_sel.setAttribute('name','selCategoryID[]');
			...
			var fb=$('<input type="button" value="&raquo;" style="color:black;">');
			var bb=$('<input type="button" value="&laquo;" style="color:black;">');
	...
	$.fn.gs_multiselect1 = function(params) {
		options = $.extend({}, $.fn.gs_multiselect1.defaults, params);
		...
		return this.each(function() {
			...
			$(fc).append($(from_sel));
			//var from_sel=fc.firstChild;
			to_sel.setAttribute('name','selAdminID[]');
			var sel_name=to_sel.getAttribute('name');			
			$(from_sel).html($(to_sel).html());
			$(this).html('');
			from_sel.style.height=to_sel.style.height=rh+'px';
