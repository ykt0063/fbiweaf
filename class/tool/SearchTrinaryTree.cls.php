...
class NodeT {
    ...
    public function __toString() {
        ...
        if ($level>1){
            ...
            else if (strcmp($checkStr,"002")==0){
                $tmpStr=$tmpStr + $this->middle + "中";
            }
            else{
     ...
     * @return number
     */
    public function getMTotalPV()
    {
        return $this->mTotalPV;
    }
    /**
     * @return number
     */
    ...
    /**
     * @param number $mTotalPV
     */
    public function setMTotalPV($mTotalPV)
    {
        $this->mTotalPV = $mTotalPV;
    }
    /**
            ...
            while(true) {
                ...
                if ($tmpStr=='001'){
                    if($current->left) {
                        ...
                        $current->left = new NodeT($info,$data);
                        break;
                    }
                    ...
                    if($current->middle) {
                        ...
                        $current->middle = new NodeT($info,$data);
                        break;
                    }
                } else {
                    if($current->right) {
                        ...
                        $current->right = new NodeT($info,$data);
                        break;
                    }
                }
            }
...
//         $this->Summary->setMiddletPV($this->root->getMTotalPV());
//         $this->Summary->setRightPV($this->root->getRTotalPV());
    ...
    private function _preorder($node,$tag=1,$bossKey=0) {
        ...
        if ($tag==2){
            ...
            //             $ediv="</div>";
            //             $href="<a href='$actual_link?menuId=7e&orgNO=$url'>";
            //             $ehref="</a>";
            ...
            if($node->middle) {
                $path2=$this->_preorder($node->middle,$tag,$bossKey);
                $node->setNewOrder($node->getNewOrder() + $node->middle->getNewOrder());
                //$node->setLTotalPV($node->left->getLTotalPV()+ $node->left->data->getPV() + $node->left->getRTotalPV()+ $node->data->getA_LINE_SUB());
                $node->setMTotalPV($node->middle->getNewOrder() + $node->data->getC_LINE_SUB());
            }
            if($node->right) {
                $path3=$this->_preorder($node->right,$tag,$bossKey);
            ...
            //            }
            if ($node->data->getLEVEL_NO_I()<3){
                $weekNo1 = Session::get('weekNo');
                
                $div="<div style='color:red; font-style:italic; font-weight:bold; font-size:80%; font-family:標楷體; width: 90px'>";
                $ediv="</div>";
                $href="<a href='$actual_link?menuID=7&orgNO=$url&WeekNo=$weekNo1'>";
                $ehref="</a>";
                ...
                //$content="會員帳號:".$node->data->getMB_NO()."<br>本期業積:".$node->data->getPV()."<br>左：".$node->getLTotalPV()."<br>中：".$node->getMTotalPV()."<br>右：".$node->getRTotalPV();
                $content="帳號:<br>".$node->data->getMB_NO()."<br>本期業積:".$node->data->getPV();
                $rowData1="{v:'".$node->info."', f:\"".$node->data->getMB_NAME()." $div $href $content $ehref $ediv \"}";
                $rowData2="'".$boss."'";
                $rowData3="''";
                //$this->path=$this->path."[$rowData1,$rowData2,$rowData3],";
            ...
            if ($node->left || $node->right|| $node->middle){
                ...
                if($node->middle) {
                    $this->_preorder($node->middle);
                    $node->setMTotalPV($node->middle->getMTotalPV()+ $node->middle->data->getPV() + $node->middle->getMTotalPV()+ $node->data->getC_LINE_SUB());
                }
                if($node->right) {
    ...
    public function BFT() {
        ...
        while(count($queue) > 0) {
            ...
            if($current_node->middle) {
                $current_node->middle->level = $current_level + 1;
                array_push($queue,$current_node->middle);
            }
